...
const ArtistSwitch = styled(Switch)(({ theme }) => ({
  width: 62,
  height: 34,
  padding: 7,
  '& .MuiSwitch-switchBase': {
...
export default function CustomizedSwitches(inputs) {
  const outputMapping = {
    "theme" : {
      true: "dark",
      false: "light"
  ...
  return (
    <FormGroup sx={{display: 'contents'}}>
      {
        inputs.switchType === 'theme' && 
        <FormControlLabel
          control={<ThemeSwitch sx={{ m: 1 }} defaultChecked onChange={valueChange}/>}
          label=""
        />
      }
      {
      ...
      {
        inputs.switchType === 'material' && 
        <FormControlLabel
          control={<MaterialUISwitch sx={{ m: 1 }} defaultChecked onChange={valueChange}/>}
          label=""
        />
      }
    </FormGroup>
